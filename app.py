...
def local_date(dt):
    """يحول الوقت المخزن (UTC) لتوقيت طرابلس (+2) ويعرضه بشكل يوم/شهر/سنة."""
    if not dt:
        return ""
...
@user_required
def wishlist():
    cutoff = now_utc() - timedelta(days=WISHLIST_FADE_DAYS)
    WishlistItem.query.filter(
        WishlistItem.done.is_(True), WishlistItem.done_at < cutoff
    ).delete(synchronize_session=False)
    db.session.commit()
...
def wishlist_toggle(item_id):
    ...
    if item.done:
        ...
        item.bought_by = session["user"]
        flash(f'تم شراء "{item.name}" - بيتحذف تلقائي بعد {WISHLIST_FADE_DAYS} أيام', "success")
    else:
        item.done_at = None
        item.bought_by = None
    db.session.commit()
